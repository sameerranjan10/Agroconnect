...
const FEATURES = [
  ...
  },
  {
    ...
    color: 'from-earth-50 to-amber-50',
    badge: 'Trusted',
  },
  {
    icon: '⚡',
    ...
    <div className="page-enter">
      ...
      <section className="relative overflow-hidden py-20 md:py-28 px-4">
        ...
        <div className="absolute inset-0 -z-10">
          <div className="absolute top-0 right-0 w-96 h-96 bg-forest-100 rounded-full blur-3xl opacity-40 -translate-y-1/2 translate-x-1/3" />
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-earth-200 rounded-full blur-3xl opacity-30 translate-y-1/2 -translate-x-1/3" />
        </div>
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          <h1 className="font-display text-4xl md:text-6xl font-bold text-stone-800 leading-tight mb-6">
            From{' '}
            <span className="text-forest-600 relative">
              Farm
            ...
            {' '}to{' '}
            <span className="text-earth-600">Market</span>,<br />Powered by AI
          </h1>
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-2">
            {CROPS.map((c) => (
              <span key={c} className="px-3 py-1.5 bg-white border border-earth-200 rounded-full text-sm text-stone-600 shadow-sm">
                {c}
              </span>
            ))}
          </div>
      ...
      {/* ── Stats ────────────────────────────────────────────────────────── */}
      <section className="py-12 border-y border-earth-100 bg-white/60">
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6">
          {STATS.map((s) => (
            <div key={s.label} className="text-center">
              <div className="text-3xl mb-1">{s.icon}</div>
              <div className="font-display text-3xl font-bold text-stone-800">{s.value}</div>
      ...
      <section className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {FEATURES.map((f) => (
              <div key={f.title} className={`rounded-2xl bg-gradient-to-br ${f.color} p-5 border border-earth-100`}>
                <div className="text-4xl mb-3">{f.icon}</div>
                <span className="badge bg-white text-forest-700 border border-forest-200 text-xs mb-2">
                  {f.badge}
                </span>
                <h3 className="font-display font-bold text-stone-800 mt-2 text-lg">{f.title}</h3>
                <p className="text-stone-500 text-sm mt-2 leading-relaxed">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="border-t border-earth-100 py-8 text-center text-stone-400 text-sm">
