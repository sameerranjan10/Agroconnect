...
const CATEGORY_COLORS = {
  ...
  Pulse:     'bg-orange-100 text-orange-800',
...
export default function ProductCard({ product, onBuy }) {
  ...
  return (
    <div className="card overflow-hidden flex flex-col group animate-fade-in">
      {/* Image / Emoji banner */}
      <div className="relative h-40 bg-gradient-to-br from-forest-50 to-earth-100 flex items-center justify-center overflow-hidden">
        {product.image_url ? (
          <img
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex-1">
          <Link to={`/products/${product.id}`}>
            <h3 className="font-display font-semibold text-stone-800 text-lg leading-snug hover:text-forest-700 transition-colors line-clamp-1">
              {product.title}
            </h3>
          </Link>
        ...
        <div className="mt-4 pt-3 border-t border-earth-100 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-sm font-medium text-stone-700">{product.quantity} {product.unit}</p>
          </div>
        </div>
