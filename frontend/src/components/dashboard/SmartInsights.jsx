    ...
    <div className="glass-card p-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {INSIGHTS.map((insight) => (
          <div key={insight.id} className="p-4 rounded-xl bg-white/5 border border-dashboard-border hover:border-emerald-500/30 transition-colors group">
            <div className="flex items-center gap-3 mb-2">
              <div className={`p-2 rounded-lg border ${insight.color}`}>
                <insight.icon className="w-4 h-4" />
              </div>
              <h4 className="font-medium text-sm text-stone-200">{insight.title}</h4>
            </div>
