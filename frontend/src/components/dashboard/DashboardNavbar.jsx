    ...
    <div className="fixed top-0 left-0 right-0 h-16 bg-white/80 TEMP_PLACEHOLDER_bg-stone-100 dark:bg-dashboard-surface/80 backdrop-blur-xl border-b border-stone-200 dark:border-dashboard-border z-40 flex items-center justify-between px-4 lg:px-8 transition-colors duration-300">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden md:flex relative">
          ...
          <input type="text" placeholder="Search..." className="bg-stone-100 dark:bg-white/5 border border-stone-200 dark:border-dashboard-border rounded-xl pl-9 pr-4 py-1.5 text-sm text-stone-700 dark:text-stone-200 placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 w-64 transition-all" />
        </div>

        {/* Theme Toggle */}
        <button onClick={toggleTheme} className="p-2 text-stone-400 dark:text-stone-500 dark:text-stone-400 hover:text-white hover:bg-stone-100 dark:bg-white/5 rounded-xl transition-colors">
          {theme === 'dark' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
        </button>

        {/* Notifications */}
        <div className="relative">
          <button onClick={() => { setShowNotifs(!showNotifs); setShowProfile(false) }} className="relative p-2 text-stone-400 dark:text-stone-500 dark:text-stone-400 hover:text-white hover:bg-stone-100 dark:bg-white/5 rounded-xl transition-colors">
          ...
          <AnimatePresence>
            {showNotifs && (
              <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="absolute right-0 mt-2 w-[calc(100vw-2rem)] sm:w-80 glass-card p-4 shadow-2xl origin-top-right">
                <h3 className="text-sm font-semibold text-stone-700 dark:text-stone-200 mb-3">Notifications</h3>
                <div className="space-y-3">
                  <div className="flex gap-3 text-sm">
                    ...
                    <div>
                      <p className="text-stone-600 dark:text-stone-300">Welcome to your new dashboard!</p>
                      <p className="text-xs text-stone-400 dark:text-stone-500">Just now</p>
                    </div>
                  </div>
                </div>
              </motion.div>
        ...
        {/* Profile */}
        <div className="relative">
          <button onClick={() => { setShowProfile(!showProfile); setShowNotifs(false) }} className="flex items-center gap-2 hover:bg-stone-100 dark:bg-white/5 rounded-full p-1 pr-3 transition-colors">
            <div className="w-8 h-8 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-sm">
              {getInitials(user?.name)}
            </div>
            <div className="hidden sm:block text-left">
              <p className="text-sm font-medium text-stone-700 dark:text-stone-200 leading-tight">{user?.name || 'User'}</p>
          ...
          <AnimatePresence>
            {showProfile && (
              <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="absolute right-0 mt-2 w-48 glass-card py-2 shadow-2xl origin-top-right">
                <div className="px-4 py-2 border-b border-stone-200 dark:border-dashboard-border/50 mb-1 sm:hidden">
                  <p className="text-sm font-medium text-stone-700 dark:text-stone-200">{user?.name}</p>
